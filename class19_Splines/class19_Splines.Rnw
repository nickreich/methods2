...
[More info: Harrel, {\em Regression Modeling Strategies}, Chapter 2, PDF handout]
...
\bi
	...
	\myitem Piecewise quadratic models are smooth and have continuous first derivatives
        \myitem Often, knots taken as quintiles of the data.
\ei
...
\begin{frame}{Advantages of piecewise models}
...
require(splines)
lm.ns3 = lm(y ~ ns(range, df=3))
plot(range, y, xlab = "Range", ylab = "log ratio", pch = 18)
points(range, lm.ns3$fitted.values, type = 'l', col = "red", lwd = 2)
@
...
lm.ns5 = lm(y ~ ns(range, df=5))
plot(range, y, xlab = "Range", ylab = "log ratio", pch = 18)
points(range, lm.ns5$fitted.values, type = 'l', col = "red", lwd = 2)
@
