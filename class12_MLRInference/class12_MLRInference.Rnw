...
\begin{figure}[t]
    \includegraphics[width=.8\textwidth]{../preambles/CircleOfLife.pdf}  
\end{figure}
...
\bi
	...
	\myitem Can we compare this model to a model with only \texttt{lMiles} and \texttt{Income}?
\ei
...
\bi
	...
	\myitem It follows that  $P(\mbox{at least one rejection} | \mbox{all } H_{0i} \mbox{are true}) = 1- (1-\alpha)^{k}$
	\myitem Further, 
	\beqa
		FWER = 1- (1-\alpha)^{k} & = & 1- \left(1-\frac{k \alpha}{k}\right)^{k} \\
				& \approx & 1- exp(1-{k \alpha}) \\
				& \approx & 1- (1-{k \alpha}) \\
				& = & {k \alpha} \\
	\eeqa
\ei
