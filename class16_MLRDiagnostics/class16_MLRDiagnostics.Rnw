...
where $p$ is the total number of independent predictors (i.e. $\beta$s) in your model.
...
\begin{frame}{Example Leverage plot with lung data}

\scriptsize
<<lungData, echo=FALSE>>=
data <- read.table("~/Google Drive/teaching/methods2/datasets/lung/lungc.txt",header=TRUE)
@

<<levPlots1, fig.height=3>>=
mlr <- lm(disease~crowding, data=data)
hii <- hatvalues(mlr)
@


<<levPlots, fig.height=3, tidy=FALSE>>=
hii <- 1:10 
x <- 1:length(hii)
