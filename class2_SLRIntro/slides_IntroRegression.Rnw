...
\title{Basic philosophy and overview of regression}
...
\bi
	...
	\ei
	\myitem Regression is an exercise in inferential statistics.
\ei
...
\begin{figure}[t]
    \includegraphics[width=.8\textwidth]{./figures/CircleOfLife.pdf}  
\end{figure}
...
\bi
	...
	\myitem We want $E(y | x) = f(x)$; we observe $y = f(x) + \epsilon$.
\ei
...
\begin{frame}{Example data: heights of mothers and daughters}
...
\begin{figure}[h]
    \includegraphics[width=.6\textwidth]{./figures/Fig01.pdf}  
\end{figure}
